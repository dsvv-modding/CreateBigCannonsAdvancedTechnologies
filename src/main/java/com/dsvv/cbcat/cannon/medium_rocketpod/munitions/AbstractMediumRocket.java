...
public abstract class AbstractMediumRocket<T extends AbstractHeavyAutocannonProjectile> extends AbstractCannonProjectile
{
    ...
    private double nextThrust;
    private byte fuel;
    protected EntityEntry<T> autocannonProjectileEntry;
    protected T projectile;

    protected AbstractMediumRocket(EntityType<? extends AbstractMediumRocket> type, Level level, double thrust, double maxThrust, EntityEntry<T> autocannonProjectileEntry) {
        super(type, level);
        this.thrust = thrust;
        this.nextThrust = thrust;
        this.maxThrust = maxThrust;
        this.autocannonProjectileEntry = autocannonProjectileEntry;
    ...
    public EntityDamagePropertiesComponent getDamageProperties()
    {
        ...
        return projectile != null ? projectile.getDamageProperties() : new EntityDamagePropertiesComponent(0, false, false, false, 0);
    }
    ...
    protected BallisticPropertiesComponent getBallisticProperties()
    {
        if (projectile == null && autocannonProjectileEntry != null)
            projectile = autocannonProjectileEntry.create(level());
        return projectile != null ? ((IAbstractAutocannonProjectileMixin)projectile).getBallisticComponent() : new BallisticPropertiesComponent(0, 0, false, 0, 0, 0, 0);
    }
        ...
        if (this.level().isClientSide || this.level().hasChunk(cpos.x, cpos.z)) {
            ...
            if (!this.isInGround()) {
                ...
                if (trailType != TrailType.NONE) {
                    int lifetime = trailType == TrailType.SHORT ? 50 : 100 + this.level().random.nextInt(30);
                    ...
                    for (int i = 0; i < 10; ++i) {
                        ...
                        if (this.fuel > 0) {
                            ParticleOptions fireOptions = new CannonSmokeParticleData(4, 0.33f, lifetime, 0.75f);
                            this.level().addAlwaysVisibleParticle(fireOptions, true, dx, dy, dz, sx, sy, sz);
                        }
                        else
                    ...
                    if (nextVelocity != null) {
                        ...
                        ParticleOptions fireOptions1 = new CannonSmokeParticleData(4, 0.33f, lifetime - 1, 0.75f);
                        Vec3 nextPos = this.position().add(nextVelocity);
                        Vec3 fireVelocity = this.nextVelocity.scale(-0.01);
                        for (int i = 0; i < 20; ++i) {
                            double partial = i * 0.1f;
                            ...
                            double sz = this.level().random.nextDouble() * 0.004d - 0.002d;
                            if (this.fuel > 0)
                                this.level().addAlwaysVisibleParticle(fireOptions1, true, dx, dy, dz, sx + fireVelocity.x, sy + fireVelocity.y, sz + fireVelocity.z);
                            else
    ...
    public void shoot(double pX, double pY, double pZ, float pVelocity, float pInaccuracy) {
        Vec3 vec3 = (new Vec3(pX, pY, pZ)).normalize().add(this.random.triangle(0.0D, 0.017 * pInaccuracy), this.random.triangle(0.0D, 0.017 * pInaccuracy), this.random.triangle(0.0D, 0.17 * pInaccuracy)).scale(pVelocity);
    ...
    @Override
    protected Vec3 getForces(Vec3 pos, Vec3 velocity) {
        Vec3 thrustVector = Vec3.ZERO;
        if (!level().isClientSide() && fuel > 0) {
            if (nextThrust < maxThrust) {
                nextThrust += thrust;
                if (nextThrust > maxThrust)
                    nextThrust = maxThrust;
            }
            thrustVector = velocity.normalize().scale(nextThrust);
        }
        return thrustVector.add(velocity.normalize().scale(-this.getDragForce()))
                .add(0.0d, this.getGravity() * (this.fuel > 0 ? 0.875f : 1.3f), 0.0d);
    }
    ...
    public void setFuel(int fuel) { this.fuel = (byte) fuel; }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        this.ageRemaining = tag.getByte("Fuel");
