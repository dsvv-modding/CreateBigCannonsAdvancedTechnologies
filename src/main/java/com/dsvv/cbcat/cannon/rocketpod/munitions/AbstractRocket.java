...
public abstract class AbstractRocket<T extends AbstractAutocannonProjectile> extends AbstractCannonProjectile
{
    ...
    protected AbstractRocket(EntityType<? extends AbstractRocket> type, Level level, double thrust, double maxThrust, EntityEntry<T> autocannonProjectileEntry) {
        super(type, level);
        this.thrust = thrust;
        ...
        if (this.level().isClientSide || this.level().hasChunk(cpos.x, cpos.z)) {
            ...
            if (!this.isInGround()) {
                ...
                if (trailType != TrailType.NONE) {
                    int lifetime = trailType == TrailType.SHORT ? 40 : 80 + this.level().random.nextInt(20);
                    ...
                    for (int i = 0; i < 10; ++i) {
                        ...
                        if (this.fuel > 0) {
                            ParticleOptions fireOptions = new CannonSmokeParticleData(2, 0.25f, lifetime, 0.66f);
                    ...
                    if (nextVelocity != null) {
                        ...
                        ParticleOptions fireOptions1 = new CannonSmokeParticleData(2, 0.25f, lifetime - 1, 0.66f);
    ...
    protected Vec3 getForces(Vec3 pos, Vec3 velocity) {
        ...
        return thrustVector.add(velocity.normalize().scale(-this.getDragForce()))
                .add(0.0d, this.getGravity() * (fuel > 0 ? 0.875f : 1.3f), 0.0d);
    }
