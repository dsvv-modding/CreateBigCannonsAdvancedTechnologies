...
public class CBCATJEI implements IModPlugin {
    private static final ResourceLocation PLUGIN_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "jei_plugin");
    @Override
    ...
    public List<RecipeHolder<CraftingRecipe>> getMunitionAssemblyRecipes() {
        ...
        for (Item result : results) {
            if (result instanceof HeavyAutocannonCartridgeItem cartridgeItem)
                recipes.add(new RecipeHolder<>(ResourceLocation.fromNamespaceAndPath(MOD_ID, result.getDescriptionId() + "_recipe"), new ShapelessRecipe("", CraftingBookCategory.MISC, cartridgeItem.getDefaultInstance(), NonNullList.of(
                    Ingredient.EMPTY, projectiles, propellant, propellant, Ingredient.of(ItemRegister.HEAVY_AUTOCANNON_EMPTY_CARTRIDGE)))));
        }
        return recipes;
    }

    public List<RecipeHolder<CraftingRecipe>> getClusterMunitionAssemblyRecipes() {
        List<RecipeHolder<CraftingRecipe>> recipes = new ArrayList<>();
        ...
        BuiltInRegistries.ITEM.stream().forEach( i -> {
            ...
            } else if (i instanceof FuzedClusterProjectileBlockItem && !(((BlockItem)i).getBlock() instanceof IProjectileCartridgeBlock))
                results.add(i);
        ...
        for (Item result : results) {
            if (result instanceof FuzedClusterProjectileBlockItem clusterItem) {
                recipes.add(new RecipeHolder<>(ResourceLocation.fromNamespaceAndPath(MOD_ID, result.getDescriptionId() + "_recipe"), new ShapelessRecipe("", CraftingBookCategory.MISC, clusterItem.getDefaultInstance(), NonNullList.of(
    ...
    public List<RecipeHolder<CraftingRecipe>> getCaselessMunitionAssemblyRecipes() {
        List<RecipeHolder<CraftingRecipe>> recipes = new ArrayList<>();
        ...
        BuiltInRegistries.ITEM.stream().forEach( i -> {
            if (i instanceof BlockItem bItem) {
                if (bItem.getBlock() instanceof IProjectileCartridgeBlock && !(BlockRegister.GRAPESHOT_CASELESS_BLOCK.is(i) || BlockRegister.GRAPESHOT_CARTRIDGE_BLOCK.is(i)))
                    results.add(i);
            }
        });
